...
axs[0, 0].bar(df.groupby('Type / Dénomination')['Valeur Marchande (USD/tonne)'].mean().sort_values().index,
              df.groupby('Type / Dénomination')['Valeur Marchande (USD/tonne)'].mean().sort_values())
axs[0, 0].set_title('Valeur marchande moyenne par type de café')
axs[0, 0].set_ylabel('Valeur marchande moyenne (USD/tonne)')
axs[0, 0].set_xticklabels(df.groupby('Type / Dénomination')['Valeur Marchande (USD/tonne)'].mean().sort_values().index, rotation=45)

# 2. Scatter plot pour l'altitude et la valeur marchande
axs[0, 1].scatter(df['Altitude (m)'], df['Valeur Marchande (USD/tonne)'], color='green', alpha=0.6)
...
sns.boxplot(x='Type / Dénomination', y='Valeur Marchande (USD/tonne)', data=df, ax=axs[0, 2], palette='Set2')
axs[0, 2].set_title('Distribution de la Valeur Marchande des Types de Café')
axs[0, 2].set_xticklabels(df['Type / Dénomination'].unique(), rotation=45)
